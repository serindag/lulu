...
class AdminBranchController extends Controller
{
    ...
    public function delete($id)
    {
        ...
        $search = User::where('branch_id', $id)->get();
        $search1 = Category::where('branch_id', $id)->get();

        if (count($search) > 0 and count($search1)) {
            return "Silme işleminin yapılabilmesi için şubenin kategorileri ve kullanıcıları silinmelidir.";
        } else {
